...
function resolve(env) {
    ...
    if (explicit === "cloudflare") {
        if (!hasCloudflare)
            throw new Error("JEV_PROVIDER=cloudflare but a Cloudflare API token (CLOUDFLARE_API_TOKEN or JEV_CLOUDFLARE_API_TOKEN) and CLOUDFLARE_ACCOUNT_ID are not both set.");
        return "cloudflare";
    }
    if (hasTypesafe)
        return "typesafe";
    ...
    if (env.AI_GATEWAY_API_KEY)
        return "vercel";
    throw new Error("No TYPESAFE_API_KEY, OPENROUTER_API_KEY (sk-or-), or Cloudflare token + CLOUDFLARE_ACCOUNT_ID found. Set one, or JEV_PROVIDER to choose explicitly.");
}
export async function askJev(state, questions, model, signal) {
    const provider = resolve(process.env);
    if (provider === "typesafe") {
        typesafeClient ??= new TypeSafeClient(process.env.TYPESAFE_BASE_URL ? { baseURL: process.env.TYPESAFE_BASE_URL } : undefined);
